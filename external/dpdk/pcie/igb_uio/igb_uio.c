...
static int
igbuio_pci_setup_iomem1(struct pci_dev *dev, struct uio_info *info,
		       int n, int pci_bar, const char *name)
{
	...
	internal_addr = dma_alloc_coherent(&dev->dev,
		(NUM_TX_DESC * 16 * 8),
		&PhyAddr, GFP_KERNEL);
	if (!internal_addr) {
		ret = -ENOMEM;
		printk("%s: alloc rx desc array failed\n", __func__);
	}

	info->mem[n].name = name;
	info->mem[n].addr = PhyAddr;
	info->mem[n].size = NUM_TX_DESC * 16 * 8;
	info->mem[n].memtype = UIO_MEM_PHYS;

	pr_err("%s %d name: %s, addr: %lld, len: %lld [%d]\n", __func__, __LINE__, name, PhyAddr, info->mem[n].size, n);
		       ...
		       int n, int pci_bar, const char *name)
{
	...
	info->mem[n].size = len;
	info->mem[n].memtype = UIO_MEM_PHYS;
	pr_err("%s %d name: %s, addr: %lx, len: %ld\n", __func__, __LINE__, name, addr, len);
...
igbuio_setup_bars(struct pci_dev *dev, struct uio_info *info)
{
	...
	//2
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "TX");
	iom++;
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "RX");
	iom++;
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "type");
	/*
	iom++;
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "type_tx");
	iom++;
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "type_rx");
	iom++;
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "DESCS");
	iom++;
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "DESCR");
	iom++;
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "DESCS1");
	iom++;
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "DESCR1");
	iom++;
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "DESCSD");
	iom++;
	ret = igbuio_pci_setup_iomem1(dev, info, iom, i, "DESCPD");
	*/
