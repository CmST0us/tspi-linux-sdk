...
static int spk_play_volume = 0;
static int hp_play_volume = 0;
static int capture_volume = 127;
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
    ...
    if (spk_play_volume >= 0) {
		temp = (spk_play_volume << 8) | spk_play_volume;
    regmap_update_bits(rt5640->regmap, RT5640_SPK_VOL,
                                        0x3f3f, temp);
    }                                   
          
	
    if (hp_play_volume >= 0) {
		temp = (hp_play_volume << 8) | hp_play_volume;
...
static int rt5640_parse_dt(struct rt5640_priv *rt5640, struct device_node *np)
{
	...
	of_property_read_u32(np,"spk-play-volume",(u32 *)&spk_play_volume);
	of_property_read_u32(np,"hp-play-volume",(u32 *)&hp_play_volume);
	of_property_read_u32(np,"capture-volume",(u32 *)&capture_volume);
...
/*	
    if (play_volume >= 0) {
		temp = (play_volume << 8) | play_volume;
    regmap_update_bits(rt5640->regmap, RT5640_DAC2_DIG_VOL,
                                        0xffff, temp);
    }                                   
*/                     
    if (capture_volume >= 0) {
		temp = (capture_volume << 8) | capture_volume;
    regmap_update_bits(rt5640->regmap, RT5640_ADC_DATA,
                                        0xffff, temp);
	}
