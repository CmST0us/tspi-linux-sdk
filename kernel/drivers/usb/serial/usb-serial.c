...
static int allocate_minors(struct usb_serial *serial, int num_ports)
{
	...
	unsigned int i, j;
	int minor;
	int ftdi_start = 0; //rpdzkj

	dev_dbg(&serial->interface->dev, "%s %d\n", __func__, num_ports);
	dev_info(&serial->interface->dev, "%s, num_port: %d, driver: %s\n", __func__, num_ports, serial->type->description);
	
	if (strcmp(serial->type->description, "FTDI USB Serial Device") == 0){
		dev_info(&serial->interface->dev, "detect %s device, minor will begin with 10\n", serial->type->description);
		ftdi_start = 10;
	}
	...
	for (i = 0; i < num_ports; ++i) {
		...
		// rpdzkj
		minor = idr_alloc(&serial_minors, port, ftdi_start,
					USB_SERIAL_TTY_MINORS, GFP_KERNEL);  //rpdzkj
		if (minor < 0)
			goto error;
		port->minor = minor;
		port->port_number = i;
