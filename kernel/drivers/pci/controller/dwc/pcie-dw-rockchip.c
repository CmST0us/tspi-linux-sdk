...
struct rk_pcie {
	...
	struct gpio_desc		*modem_pwr_gpio;
	struct gpio_desc		*modem_en_gpio;
	struct gpio_desc		*pcie_waken_gpio;
...
static int rk_pcie_establish_link(struct dw_pcie *pci)
{
	...
	msleep(rk_pcie->perst_inactive_ms);
	 gpiod_set_value_cansleep(rk_pcie->modem_pwr_gpio, 0);
	 gpiod_set_value_cansleep(rk_pcie->modem_en_gpio, 0);
	 gpiod_set_value_cansleep(rk_pcie->pcie_waken_gpio, 0);
	 msleep(50);
	 
	 gpiod_set_value_cansleep(rk_pcie->modem_pwr_gpio, 1);
	 gpiod_set_value_cansleep(rk_pcie->modem_en_gpio, 1);
	 gpiod_set_value_cansleep(rk_pcie->pcie_waken_gpio, 1);
	 msleep(50);
					 ...
					 struct rk_pcie *rk_pcie)
{
	...
	rk_pcie->modem_pwr_gpio = devm_gpiod_get_optional(&pdev->dev, "modem-pwr",
						    GPIOD_OUT_LOW);
	if (IS_ERR(rk_pcie->modem_pwr_gpio)) {
		dev_err(&pdev->dev, "invalid modem-pwr-gpios property in node\n");
		return PTR_ERR(rk_pcie->modem_pwr_gpio);
	}
	rk_pcie->modem_en_gpio = devm_gpiod_get_optional(&pdev->dev, "modem-en",
						    GPIOD_OUT_LOW);
	if (IS_ERR(rk_pcie->modem_en_gpio)) {
		dev_err(&pdev->dev, "invalid modem-en-gpios property in node\n");
		return PTR_ERR(rk_pcie->modem_en_gpio);
	}
	rk_pcie->pcie_waken_gpio = devm_gpiod_get_optional(&pdev->dev, "pcie-waken",
						    GPIOD_OUT_LOW);
	if (IS_ERR(rk_pcie->pcie_waken_gpio)) {
		dev_err(&pdev->dev, "invalid pcie-waken-gpios property in node\n");
		return PTR_ERR(rk_pcie->pcie_waken_gpio);
	}
