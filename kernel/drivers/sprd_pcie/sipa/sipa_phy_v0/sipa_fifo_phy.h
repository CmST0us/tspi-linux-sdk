...
#define IPA_TFT_COMMON_DATA_FILLED_DEPTH 0x165UL
#define IPA_TFT_COMMON_DATA_FILLED_LIMIT_DEPTH 0x264UL
#define IPA_TFT_COMMON_DATA_FILLED_CP0_FREE_DEPTH 0x308UL
#define IPA_TFT_COMMON_DATA_FILLED_CP1_FREE_DEPTH 0x368UL
...
static inline u32 ipa_phy_get_tft_data_filled_depth(void __iomem *tft_fifo_base)
{
   u32 tmp;

   tmp = readl_relaxed(tft_fifo_base + IPA_TFT_COMMON_DATA_FILLED_DEPTH);

   return 2 << (tmp & 0x0000000FUL);
}

static inline u32 ipa_phy_get_tft_data_filled_cp0_free_depth(void __iomem *tft_fifo_base)
{
   u32 tmp;

   tmp = readl_relaxed(tft_fifo_base + IPA_TFT_COMMON_DATA_FILLED_CP0_FREE_DEPTH);

   return (tmp >> 16) & 0x0000FFFFUL;
}

static inline u32 ipa_phy_get_tft_data_filled_cp1_free_depth(void __iomem *tft_fifo_base)
{
   u32 tmp;

   tmp = readl_relaxed(tft_fifo_base + IPA_TFT_COMMON_DATA_FILLED_CP1_FREE_DEPTH);

   return (tmp >> 16) & 0x0000FFFFUL;
}

static inline u32 ipa_phy_get_tft_data_filled_limit_depth(void __iomem *tft_fifo_base)
{
   u32 tmp;

   tmp = readl_relaxed(tft_fifo_base + IPA_TFT_COMMON_DATA_FILLED_LIMIT_DEPTH);
   tmp &= 0x0000FFFFUL;
   return tmp;
}
