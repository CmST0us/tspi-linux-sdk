				    ...
				    enum sipa_evt_type evt)
{
    ...
    if(SIPA_LEAVE_FLOWCTRL == evt){
        ...
        /* Non direct connection mode: Flow control scheme
				  ...
				  u32 data)
{
	...
	if (evt & SIPA_IRQ_EXIT_FLOW_CTRL) {
        ...
        //pr_info("sipa_sender_notify_cb set ep_cover_net false!!!!!\n");
...
#if defined (__BIG_ENDIAN_BITFIELD)
		if (node.err_code)
			dev_err(ctrl->dev, "have node transfer err = %d, i=%d, num=%d\n",
				node.err_code, i, num);
#else
		if (node->err_code)
			dev_err(ctrl->dev, "have node transfer err = %d, i=%d, num=%d\n",
...
static bool sipa_sender_ck_unfree(struct sipa_skb_sender *sender)
{
    ...
    if (!ctrl->remote_ready) {
        pr_err("%s: need wait remote_ready!\n", __func__);
		return false;
    }
...
static int sipa_free_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
        ...
        /* Non direct connection mode: Flow control scheme
        if (sender->ep_cover_net){
            usleep_range(100, 200);
            //pr_info("%s,  still in flow ctrl\n", __func__);
        }
        */
	}
			      ...
			      u8 netid)
{
	...
	u32 tft_cp0_free_node = 0;
	u32 tft_cp1_free_node = 0;
	static u32 tft_limit_depth = 0;
#endif
	
    spin_lock_irqsave(&sender->send_lock, flags);
#ifndef CONFIG_SPRD_ETHERNET
    if (tft_limit_depth == 0){
        tft_limit_depth = ipa_phy_get_tft_data_filled_limit_depth(ctrl->tft_virt_reg_addr);
        tft_limit_depth = tft_limit_depth + 200;
		pr_info("%s, when %d free nodes left for tft, wll limit host send skb!!!!!\n", __func__, tft_limit_depth); 
    }

    tft_cp0_free_node = ipa_phy_get_tft_data_filled_cp0_free_depth(ctrl->tft_virt_reg_addr);
    tft_cp1_free_node = ipa_phy_get_tft_data_filled_cp1_free_depth(ctrl->tft_virt_reg_addr);
    if((tft_cp0_free_node+tft_cp1_free_node)< tft_limit_depth){
	    sender->tft_node_flowctl++;
		spin_unlock_irqrestore(&sender->send_lock, flags);
		return -EAGAIN;
    }
#endif
