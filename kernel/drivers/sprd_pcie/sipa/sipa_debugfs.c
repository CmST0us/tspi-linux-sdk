...
static int sipa_params_debug_show(struct seq_file *s, void *unused)
{
    ...
    u32 tft_cp0_free_node, tft_cp1_free_node;
    u32 limit_node_num;
	...
	seq_printf(s, "remote ready = %d tft_reg_mapped = 0x%llx tft_virt_reg_addr = 0x%p\n",
		   ipa->remote_ready, (long long unsigned int)ipa->tft_reg_mapped, ipa->tft_virt_reg_addr);
    ...
    tft_cp0_free_node = ipa_phy_get_tft_data_filled_cp0_free_depth(ipa->tft_virt_reg_addr);
    tft_cp1_free_node = ipa_phy_get_tft_data_filled_cp1_free_depth(ipa->tft_virt_reg_addr);
    limit_node_num = ipa_phy_get_tft_data_filled_limit_depth(ipa->tft_virt_reg_addr);
    
	seq_printf(s, "tft_cp0_free_node=%d, tft_cp1_free_node=%d, limit_node_num=%d\n", tft_cp0_free_node, tft_cp1_free_node, limit_node_num);
	...
	seq_printf(s, "sender enter_flow_ctrl_cnt=%d, exit_flow_ctrl_cnt=%d, free_notify_net=%d, ep_cover_net=%d, tft_node_flowctl=%d\n",
		   ipa->sender->enter_flow_ctrl_cnt, ipa->sender->exit_flow_ctrl_cnt,
		   ipa->sender->free_notify_net, ipa->sender->ep_cover_net, ipa->sender->tft_node_flowctl);
