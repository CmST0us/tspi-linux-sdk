...
rk628_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!rk628)
		return -ENOMEM;
printk ("123abc");
	clk_soc_24M = devm_clk_get(dev, "soc_24M");
	if (IS_ERR(clk_soc_24M)) {

printk ("123a");
		ret = PTR_ERR(clk_soc_24M);
		dev_err(dev, "failed to get clk_soc_24M: %d\n", ret);
		return ret;

	}
	clk_prepare_enable(clk_soc_24M);
